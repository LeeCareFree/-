...
Component({
    ...
    lifetimes: {
        ...
        attached: function() {
            this.triggerEvent('onCallbackDate', {
                date: this.data.callbackDate
            });
            // 在组件实例进入页面节点树时执行
        },
        detached: function() {
                ...
                case 'weeks':
                    {
                        ...
                        let startMonth = change(starDate.getMonth() + 1);
                        let startDay = change(starDate.getDate());
                        let endDate = new Date(date.getTime());
                        if (nextEable) {
                            endDate.setDate(date.getDate() + (6 - weeksDay));
                        }
                        let endYear = endDate.getFullYear();
                        let endMonth = change(endDate.getMonth() + 1);
                        let endDay = change(endDate.getDate());
                ...
                case 'quarter':
                    {
                        ...
                        callbackDate.curDate = content;
                        callbackDate.startDate = startYear + "-" + change(startMonth) + "-" + change(startDay);
                        callbackDate.endDate = endYear + "-" + change(endMonth) + "-" + change(endDay);
                        break;
                    }
                case 'custom':
                    {
